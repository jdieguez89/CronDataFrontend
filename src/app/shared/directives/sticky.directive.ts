...
export class StickyDirective implements OnInit, AfterViewInit, OnDestroy {
  ...
  @Input('spacerElement') spacerElement!: HTMLElement;
  // tslint:disable-next-line
  @Input('boundaryElement') boundaryElement!: HTMLElement;
  ...
  constructor(private stickyElement: ElementRef, @Inject(PLATFORM_ID) private platformId: string) {
    ...
    // @ts-ignore
    this.resizeThrottled$ = this.resize$.pipe(throttleTime(0, animationFrame), startWith(null), share());
  ...
  private determineStatus(originalVals: StickyPositions,
    ...
    const reachedLowerEdge = this.boundaryElement &&
      ...
      ((originalVals.bottomBoundary ? originalVals.bottomBoundary : 1000) - marginTop);
    return {
